// node_modules/stream-chat/dist/esm/index.js
...
var MentionsSearchSource = class extends BaseSearchSource {
  constructor(channel, options) {
    ...
    this.queryUsers = async (searchQuery) => {
      const { filters, sort, options: options2 } = this.prepareQueryUsersParams(searchQuery);
      const { users } = await this.client.queryUsers(filters, sort, options2);
      return users;
    };
    this.queryMembers = async (searchQuery) => {
      const { filters, sort, options: options2 } = this.prepareQueryMembersParams(searchQuery);
      const response = await this.channel.queryMembers(filters, sort, options2);
...
var ChannelManager = class extends WithSubscriptions {
  constructor({
    ...
    this.setOptions = (options2 = {}) => {
      this.options = { ...DEFAULT_CHANNEL_MANAGER_OPTIONS, ...options2 };
    };
    this.executeChannelsQuery = async (payload, retryCount = 0) => {
      const { filters, sort, options: options2, stateOptions } = payload;
      const { offset, limit } = {
      ...
      try {
        ...
        const newOptions = { ...options2, offset: newOffset };
        const { pagination } = this.state.getLatestValue();
        this.state.partialNext({
          channels,
          pagination: {
    ...
    this.queryChannels = async (filters, sort = [], options2 = {}, stateOptions = {}) => {
      const {
      ...
      const executeChannelsQueryPayload = { filters, sort, options: options2, stateOptions };
      try {
    ...
    this.loadNext = async () => {
      ...
      const { filters, sort, options: options2, isLoadingNext, hasNext } = pagination;
...
//# sourceMappingURL=chunk-S3LM2PJU.js.map
